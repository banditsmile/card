...
class HomeController extends Controller
{
				...
				public function Reg( )
				{
								...
								{
												$this->Alert( "您好，您还未退出系统，系统将自动退出并进入注册页面！" );
				...
				public function AgentLogin( $R5d899a20a5, $R00bb5d9597, $Re5584ed794 )
				{
								...
								if ( $R4e420efcc3 == 0 )
								{
												$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "用户名不存在，登陆失败", 0 );
												$this->Alert( "商家不存在，请确认是否输入错误或者登陆类型选择错误" );
												$this->HistoryGo( );
								}
								else if ( $R4e420efcc3 == 2 )
								{
												$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "客户未开通，登陆失败", 0 );
												$this->Alert( "您的帐户系统还没有开通或者已经被冻结！请联系您的上级代理商" );
												$this->HistoryGo( );
								}
								else if ( $R4e420efcc3 == 1 )
								{
												$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "密码错误，登陆失败", 0 );
												$this->Alert( "密码输入有误" );
												$this->HistoryGo( );
								}
								...
								else
								{
												...
												{
																$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "设置了硬件绑定，非本机登陆，登陆失败", 0 );
																$this->Alert( "您设置硬件绑定登录，请确认登陆时候是否勾选了硬件绑定\\n如果已经勾选，请找到您设置绑定的电脑登录即可！" );
																$this->HistoryGo( );
												}
												else
												{
																...
																{
																				$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "异常错误，登陆失败", 0 );
																				$this->Alert( "异常错误" );
																				$this->HistoryGo( );
																}
																...
																{
																				$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "密保输入错误，登陆失败", 0 );
																				$this->Alert( "密保输入错误，请重新输入" );
																				$this->HistoryGo( );
																}
																if ( $this->VerifyMobile( $R3db8f5c8bc['aid'], 0 ) == 0 )
																{
																				$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "令牌输入错误，登陆失败", 0 );
																...
																{
																				$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "客户级别有问题，可能是系统设置有问题，登陆失败", 0 );
																				$this->Alert( "您的级别有问题，请和客服联系！" );
																				$this->HistoryGo( );
																}
																if ( $R046b4585a2['gid'] == 0 )
																{
																				$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "会员用户无法登陆经销商系统，登陆失败", 0 );
																...
																if ( 0 < $R3db8f5c8bc['parentid'] )
																{
																				...
																				{
																								$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "上家不存在，登陆失败", 0 );
																								$this->Alert( "您的上家已经不存在，请联系平台客服咨询！" );
																								$this->HistoryGo( );
																				}
																				else if ( $R9aa102385f['frozen'] == 1 )
																				{
																								$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "上家被冻结，登陆失败", 0 );
																				...
																				{
																								$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "上家级别有问题，登陆失败", 0 );
																				...
																				if ( $R4420266e85 )
																				{
																								...
																								{
																												$this->AddLogin( 0, 0, $R5d899a20a5, 0, 1, "上家或被降级，登陆失败", 0 );
																...
																$this->AddLogin( $R3db8f5c8bc['aid'], 0, $R3db8f5c8bc['aname'], 0, 1, "成功登陆", 1 );
				...
				public function SetWebCookie( $R2a51483b14, $R70e13706d8 = 0, $R94e0136c8a = 0 )
				{
								...
								{
												$this->Alert( "您好，您已经设置不允许通过网页方式登录！" );
												$this->HistoryGo( );
								}
				...
				public function StaffLogin( $R0dc0347d49, $Rc509b4f207, $Re5584ed794 )
				{
								...
								if ( $R4e420efcc3 == 0 )
								{
												$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "员工账号不存在，登陆失败", 0 );
												$this->Alert( "员工不存在，请确认是否输入错误或者登陆类型选择错误" );
												$this->StaffHistoryGo( );
								}
								else if ( $R4e420efcc3 == 2 )
								{
												$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "账号被冻结，登陆失败", 0 );
												$this->Alert( "老板还没有开通您的员工帐号！或者您的老板已经被冻结！请联系您的老板" );
												$this->StaffHistoryGo( );
								}
								else if ( $R4e420efcc3 == 1 )
								{
												$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "密码输入有误，登陆失败", 0 );
												$this->Alert( "密码输入有误" );
												$this->StaffHistoryGo( );
								}
								else if ( $R4e420efcc3 == 7 )
								...
								else
								{
												if ( $R4e420efcc3 == 3 || $R4e420efcc3 == 4 || $R4e420efcc3 == 5 || $R4e420efcc3 == 6 )
												{
																$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "设置了硬件绑定，非本机登陆，登陆失败", 0 );
																$this->Alert( "您设置硬件绑定登录，请确认登陆时候是否勾选了硬件绑定\\n如果已经勾选，请找到您设置绑定的电脑登录即可！" );
																$this->StaffHistoryGo( );
												}
												else
												{
																...
																{
																				$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "异常错误，登陆失败", 0 );
																				$this->Alert( "异常错误" );
																				$this->StaffHistoryGo( );
																}
																...
																{
																				$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "密保输入错误，登陆失败", 0 );
																				$this->Alert( "密保输入错误，请重新输入" );
																				$this->StaffHistoryGo( );
																}
																if ( $this->VerifyMobile( $R3db8f5c8bc['aid'], $Raac42e4217['staffid'] ) == 0 )
																{
																				$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "令牌输入错误，登陆失败", 0 );
																				$this->Alert( "令牌输入错误，请重新输入" );
																				$this->GoHome( "b2b" );
																}
																$R2097a8fddf = factory::getinstance( "agents" );
																...
																if ( !isset( $R046b4585a2['name'] ) )
																{
																				$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "客户级别有问题，可能是系统设置有问题，登陆失败", 0 );
																				$this->Alert( "您的级别有问题，请和客服联系！" );
																				$this->HistoryGo( );
																}
																if ( $R046b4585a2['gid'] == 0 )
																{
																				$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "会员用户无法登陆经销商系统，登陆失败", 0 );
																				$this->Alert( "会员用户无法登陆经销商系统！" );
																				$this->HistoryGo( );
																}
																setcookie( "umebiz_com_gid", "", time( ) - 3600 );
																...
																if ( 0 < $R3db8f5c8bc['parentid'] )
																{
																				...
																				if ( !isset( $R9aa102385f['frozen'] ) )
																				{
																								$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "上家不存在，登陆失败", 0 );
																								$this->Alert( "您的上家已经不存在，请联系平台客服咨询！" );
																								$this->HistoryGo( );
																				}
																				else if ( $R9aa102385f['frozen'] == 1 )
																				{
																								$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "上家被冻结，登陆失败", 0 );
																								$this->Alert( "您的上家已经被冻结，请联系您的上家！" );
																								$this->HistoryGo( );
																				}
																				$R4906cf6137 = $this->GetRank( $R9aa102385f['alv'] );
																				if ( !isset( $R4906cf6137['name'] ) )
																				{
																								$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "上家级别有问题，登陆失败", 0 );
																								$this->Alert( "您的上家级别有问题，请联系您的上家！" );
																								$this->HistoryGo( );
																				}
																				$R4420266e85 = $this->GetRankType( );
																				if ( $R4420266e85 )
																				{
																								if ( $R4906cf6137['gid'] <= $R046b4585a2['gid'] )
																								{
																												$this->AddLogin( 0, 1, $R0dc0347d49, 0, 1, "上家或被降级，登陆失败", 0 );
																												$this->Alert( "您的上家或被降级，导致您的级别出现问题，无法登陆，请联系您的上家！" );
																												$this->HistoryGo( );
																								}
																				}
																...
																$this->AddLogin( $R3db8f5c8bc['aid'], $Raac42e4217['staffid'], $Raac42e4217['staffname'], 0, 1, "成功登陆", 1 );
				...
				public function Suggest( )
				{
								...
								{
												$this->Alert( "格式不正确，请按照格式填写！" );
												$this->CloseWin( );
								}
								$R2a51483b14 = intval( $Rcc5c6e696c[2] );
								if ( $R2a51483b14 == 0 )
								{
												$this->Alert( "经销商错误，请填写您的编号！" );
												$this->CloseWin( );
								}
								if ( intval( $Rcc5c6e696c[1] ) == 0 )
								{
												$this->Alert( "面值错误，请按照格式填写！" );
												$this->CloseWin( );
								}
								$R1ed7ad9990 = ACACHE."aid_".$R2a51483b14.".php";
								if ( !file_exists( $R1ed7ad9990 ) )
								{
												$this->Alert( "系统目前不暂接收非本平台经销商的提议！谢谢您对我们的支持" );
												$this->CloseWin( );
								}
								if ( $R36130a8639 == 1 )
								{
												$R06c518f70e = "[平台没有点卡]".$Re82ee9b121;
								}
								else
								{
												$R06c518f70e = "[平台点卡错误]".$Re82ee9b121;
								}
								$data = array(
												"msgfrom" => $R2a51483b14,
												"msgto" => 0,
								...
								$this->Alert( "谢谢您给我们提交意见！我们会及时处理的！" );
								$this->CloseWin( );
				}

...
class phpmail extends mysql
{
				...
				public function IMail_CReg( $data = array( ) )
				{
								...
								$mail->MsgHTML( $body );
								$address = $data['cmail'];
								$mail->AddAddress( $address, $rs['webname']."��Ա" );
								if ( !$mail->Send( ) )
								{
												return 1;
								}
				...
				public function IMail_CBuy( $data = array( ) )
				{
								...
								if ( $orderrs['ptype'] == 0 || $orderrs['ptype'] == 3 || $orderrs['ptype'] == 100 || $orderrs['ptype'] == 101 )
								{
												...
												{
																$result .= "���ţ�".$item['cardnumber']."   ���룺".base64_decode( $item['cardpwd'] )."<br/>";
												}
								}
								else
								{
								...
								$ptype = $orderrs['ptype'] == 0 || $orderrs['ptype'] == 3 || $orderrs['ptype'] == 100 || $orderrs['ptype'] == 101 ? "������" : "�ǿ�����";
								$mail = new PHPMailer( );
								...
								$mail->MsgHTML( $body );
								$address = $data['cmail'];
								$mail->AddAddress( $address, $rs['webname']."�ͻ�" );
								if ( !$mail->Send( ) )
								{
												return 1;
								}
				...
				public function IMail_AReg( $data = array( ) )
				{
								...
								$mail->MsgHTML( $body );
								$address = $data['amail'];
								$mail->AddAddress( $address, $rs['webname']."������" );
								if ( !$mail->Send( ) )
								{
												return 1;
								}

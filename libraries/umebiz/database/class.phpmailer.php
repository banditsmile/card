...
class PHPMailer
{
				...
				public function Send( )
				{
								try
								{
												...
												{
																throw new self( $this->Lang( "provide_address" ), self::STOP_CRITICAL );
												}
												...
												if ( empty( $this->Body ) )
												{
																throw new self( $this->Lang( "empty_message" ), self::STOP_CRITICAL );
												}
												switch ( $this->Mailer )
												{
												case "sendmail" :
				...
				protected function SendmailSend( $header, $body )
				{
								...
								{
												throw new self( $this->Lang( "execute" ).$this->Sendmail, self::STOP_CRITICAL );
								}
								fputs( $mail, $header );
								fputs( $mail, $body );
								$result = pclose( $mail );
				...
				protected function MailSend( $header, $body )
				{
								...
								if ( !$rt )
								{
												throw new self( $this->Lang( "instantiate" ), self::STOP_CRITICAL );
								}
								return true;
				}
				...
				protected function SmtpSend( $header, $body )
				{
								...
								if ( !$this->SmtpConnect( ) )
								{
												throw new self( $this->Lang( "smtp_connect_failed" ), self::STOP_CRITICAL );
								}
								$smtp_from = $this->Sender == "" ? $this->From : $this->Sender;
								if ( !$this->smtp->Mail( $smtp_from ) )
								{
												throw new self( $this->Lang( "from_failed" ).$smtp_from, self::STOP_CRITICAL );
								}
								...
								{
												throw new self( $this->Lang( "data_not_accepted" ), self::STOP_CRITICAL );
								}
				...
				public function AddAttachment( $path, $name = "", $encoding = "base64", $type = "application/octet-stream" )
				{
								try
								{
												if ( !is_file( $path ) )
												{
																throw new self( $this->Lang( "file_access" ).$path, self::STOP_CONTINUE );
												}
				...
				private function EncodeFile( $path, $encoding = "base64" )
				{
								try
								{
												if ( !is_readable( $path ) )
												{
																throw new self( $this->Lang( "file_open" ).$path, self::STOP_CONTINUE );
												}
				...
				public function Base64EncodeWrapMB( $str )
				{
								...
								$encoded = substr( $encoded, 0, 0 - strlen( $this->LE ) );
								return $encoded;
				}
				...
				public function set( $name, $value = "" )
				{
								try
								{
												...
												{
																throw new self( $this->Lang( "variable_set" ).$name, self::STOP_CRITICAL );
												}
								}
								catch ( Exception $e )
								{

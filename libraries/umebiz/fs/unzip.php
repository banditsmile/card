...
class unzip
{
				...
				public function extract_files( $zipname, $dst )
				{
								$zip = new dUnzip2( $zipname );
								$zip->debug = false;
								$zip->getList( );
								$zip->unzipAll( $dst );
				}
...
class dUnzip2
{
				...
				public $compressedList = NULL;
				public $centralDirList = NULL;
				public $endOfCentral = NULL;
				public $debug = NULL;
				public $fh = NULL;
				public $zipSignature = "PK\x03\x04";
				public $dirSignature = "PK\x01\x02";
				public $dirSignatureE = "PK\x05\x06";
				...
				public function unzip( $compressedFileName, $targetFileName = false, $applyChmod = 493 )
				{
								if ( !sizeof( $this->compressedList ) )
								{
								...
								$ret = $this->uncompress( $toUncompress, $fdetails['compression_method'], $fdetails['uncompressed_size'], $targetFileName );
								unset( $toUncompress );
								if ( $applyChmod && $targetFileName )
								{
												chmod( $targetFileName, 493 );
								}
								return $ret;
				}

				public function unzipAll( $targetDir = false, $baseDir = "", $maintainStructure = true, $applyChmod = 493 )
				{
								if ( $targetDir === false )
								{
				...
				public function _loadFileListByEOF( &$fh, $stopOnFile = false )
				{
								...
								for ( $x = 0;	$x < 1024;	$x++	)
								{
												...
												if ( $signature == $this->dirSignatureE )
												{
																...
																$eodir['zipfile_comment'] = $zipFileCommentLenght[1] ? fread( $fh, $zipFileCommentLenght[1] ) : "";
																...
																while ( $signature == $this->dirSignature )
																{
																				...
																				$extraFieldLength = unpack( "v", fread( $fh, 2 ) );
																				...
																				$dir['file_name'] = fread( $fh, $fileNameLength[1] );
																				$dir['extra_field'] = $extraFieldLength[1] ? fread( $fh, $extraFieldLength[1] ) : "";
				...
				public function _getFileHeaderInformation( &$fh, $startOffset = false )
				{
								...
								if ( $signature == $this->zipSignature )
								{
												...
												$fileNameLength = unpack( "v", fread( $fh, 2 ) );
												$extraFieldLength = unpack( "v", fread( $fh, 2 ) );
												$file['file_name'] = fread( $fh, $fileNameLength[1] );
												$file['extra_field'] = $extraFieldLength[1] ? fread( $fh, $extraFieldLength[1] ) : "";
				...
				{
								$from = "·µ¶Ç’€ÔÒÓŞÖ×ØÑ¥ãàâå™ëéêšíèá… ƒÆ„†‘‡Š‚ˆ‰¡Œ‹Ğ¤•¢“ä”›—£–ìç˜ï";

...
<div id="titleDiv">
<div style="float:left"><a href="index.php?a=Home"><img src="<?php echo $vd['sc']; ?>images/home.png" style="vertical-align:middle" border="0"/></a></div><div style="float:left;padding-top:8px;padding-left:3px;"><a href="index.php?a=Home" title="回到后台首页"><font color="#000">桌面</font></a> <span style="font-size:7px;">>></span> <a href="index.php?m=mod_b2b&c=Article" title="文章列表"><font color="#000">文章列表</font></a> <span style="font-size:7px;">>></span> <span style="font-size:12px;"><?php echo $item['title']; ?></span></div>
<div style="float:right;"><a href="index.php?m=mod_home&a=Help&t=admin_b2b_article_detail" onFocus="this.blur()" title="查看文章相关帮助"><img src="<?php echo $vd['sc']; ?>images/help.gif" style="vertical-align:middle" border="0"/></a></div>
</div>
...
<div>
  <table border="0" class="ctable" bordercolor="#ededed">
    <tr>
      ...
      <td class="tablert">
        <input name="title" type="text" class="input" size="40" value="<?php echo $item['title']; ?>">
        <input type="checkbox" class="checkbox" onclick="setdisplay(this)" onFocus="this.blur()" id="istitlelink"/> 标题转向其它地址
        <input type="checkbox" class="checkbox" name="stick" value="1" onFocus="this.blur()" <?php (ToggleCheck($item['stick'])); ?>/> 置顶
        <input type="checkbox" class="checkbox" name="hidden" value="1" onFocus="this.blur()" <?php (ToggleCheck($item['hidden'])); ?>/> 隐藏
      </td>
    </tr>
    <tr> 
      <td class="tablelt">
      ...
      <td class="tablert">
        <input type="text" id="titlecolor" name="titlecolor" value="<?php echo $item['titlecolor']; ?>" size="10" onkeyup="setcolor(this)"/> 
        <input id="colorexample" type="text" size="1" readonly style="background:<?php if($item['titlecolor']==''){ ?>#ccc<?php }else{ ?>$item['titlecolor']<?php } ?>">
    ...
    <tr id="titlelink" style="display:none"> 
      <td class="tablelt">
        <strong onmouseover="showhide(this);" onmouseout="showhide(this);">[?]</strong><p>前台用户点击文章标题后，会直接转入这个地址，如果不想转向，务必留空</p>
        转向地址：
      </td>
      <td class="tablert">
        <input name="titlelink" type="text" class="input" size="50" value="<?php echo $item['titlelink']; ?>"> 
      </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td class="tablelt">
...
<div>
  <table border="0" id="ctable2" class="ctable" bordercolor="#ededed">
    <tr> 
      ...
      <td class="tablert"><input type="text" name="webtitle" value="<?php echo $item['webtitle']; ?>" size="34"/> <span class="spantip">(留空则默认显示文章主题)</span></td>
    </tr>
    <tr> 
      <td class="tablelt">
        META_KEYWORDS(页面关键词)
      ...
      <td class="tablert">
        <textarea rows="3" name="meta_keywords" cols="28" style="width:195px;"><?php echo $item['meta_keywords']; ?></textarea>
    ...
    <tr> 
      ...
      <td class="tablert">
        <textarea rows="3" name="meta_description" cols="28" style="width:195px;"><?php echo $item['meta_description']; ?></textarea>
  ...
  <table border="0" id="ctable2" class="ctable" bordercolor="#ededed" style="border:0px">
    <tr> 
      <td style="padding:0px;overflow-y:hidden">
        <textarea name="nr" cols="67" rows="2" style="display:none"><?php echo eregi_replace('\\\%', "%", $item['content']); ?></textarea>
...
<div id="opcontent">
  ...
  <div class="optxt">
    ...
    <input type="submit" value="编辑文章" class="btn"/>
...
var titlehaslink = <?php if($item['titlelink']==""){ ?>0<?php }else{ ?>1<?php } ?>;
$("titlelink").style.display = titlehaslink ? "" : "none";
$("mycontent").style.display = titlehaslink ? "none" : "";
$("seo").style.display = titlehaslink ? "none" : "";

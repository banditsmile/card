...
$tabletpl = array(
				...
				"view" => array( "width" => "22", "headHtml" => "<img src=\"{\$vd['sc']}images/icon_view.gif\"/>", "bodyHtml" => "<!--if(\$item['ubzuida']==\$item['ubzuidb']){--><font color=\"#cccccc\">无</font><!--}else{--><a href=\"index.php?m=mod_b2b&c=order&a=detail&ubzordno={\$item['ubzordno']}&ishistory=<!--echo intval(request('ishistory'));-->\" title=\"查看订单\" onFocus=\"this.blur()\" onclick=\"loadDisp(1)\"><img src=\"{\$vd['sc']}images/icon_view.gif\"/></a><!--}-->" ),
				"orddate" => array( "width" => "140", "headHtml" => "<b>交易时间</b>", "bodyHtml" => "{\$item['ubztrddate']}" ),
				"expend" => array( "width" => "65", "headHtml" => "<b>支出</b>", "bodyHtml" => "<!--if(\$item['ubzincome'] != \$item['ubzexpend'] && \$item['ubzexpend'] != 0){-->- {\$item['ubzexpend']}<!--}else if(\$item['ubzexpend'] == 0 && \$item['ubzincome'] == 0){-->- {\$item['ubzexpend']}<!--}-->" ),
				"income" => array( "width" => "65", "headHtml" => "<b>收入</b>", "bodyHtml" => "<!--if(\$item['ubzincome'] != \$item['ubzexpend'] && \$item['ubzincome'] != 0){-->+ {\$item['ubzincome']}<!--}else if(\$item['ubzexpend'] == 0 && \$item['ubzincome'] == 0){-->+ {\$item['ubzincome']}<!--}-->" ),
				"remain" => array( "width" => "70", "headHtml" => "<b>帐户余额</b>", "bodyHtml" => "{\$item['ubzremain']}" ),
